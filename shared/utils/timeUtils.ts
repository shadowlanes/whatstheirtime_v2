...
export interface DayNightDisplay {
  icon: string;
  label: string;
  className: string;
  cardGradient: string;
 ...
 * @param offsetMinutes - Minutes to offset (can be negative)
 * @returns Luxon DateTime object
 */
export function getOffsetTime(timezoneId: string, offsetMinutes: number = 0): DateTime {
  return DateTime.now()
    .setZone(timezoneId)
 ...
 * @returns One of the time-of-day phases
 */
export function getDayNightState(dt: DateTime): DayNightState {
  const hour = dt.hour;
 ...
 * @param state - One of the time-of-day phases
 * @returns Display configuration object
 */
export function getDayNightDisplay(state: DayNightState): DayNightDisplay {
  const displays: Record<DayNightState, DayNightDisplay> = {
    'deep-night': {
      icon: '🌙',
      label: 'Deep Night',
 ...
 * @param friendTime - Friend's current time
 * @param userTime - User's current time (with offset)
 * @returns Smart formatted date
 */
export function formatSmartDate(friendTime: DateTime, userTime: DateTime): string {
 ...
 * @returns Formatted offset (e.g., "+2h 30m", "Now", "-1h")
 */
